...
const SalesPage: React.FC = () => {
  ...
  const handleTranscript = async (transcript: string) => {
    ...
    setVoiceError(null);
    try {
        const result: ParsedSaleFromAI = await parseSaleFromVoice(transcript, mockProducts);
        
        const saleItems: CartItem[] = [];
        let saleTotal = 0;

        result.items.forEach(item => {
            const product = mockProducts.find(p => p.name.toLowerCase() === item.productName.toLowerCase());
            if (product) {
